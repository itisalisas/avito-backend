...
func (r *ReceptionRepository) AddReception(ctx context.Context, reception *dto.Reception, tx *sql.Tx) error {
	...
	err = tx.QueryRowContext(ctx, query, args...).Scan(&reception.Id, &reception.DateTime, &reception.Status)

	return err
}

func (r *ReceptionRepository) GetLastReceptionByPvzId(ctx context.Context, pvzId openapi_types.UUID, tx *sql.Tx) (*dto.Reception, error) {
	...
	err = tx.QueryRowContext(ctx, query, args...).Scan(&reception.Id, &reception.DateTime, &reception.Status, &reception.PvzId)
	switch {
...
func (r *ReceptionRepository) CloseLastReception(ctx context.Context, receptionId openapi_types.UUID, tx *sql.Tx) (*dto.Reception, error) {
